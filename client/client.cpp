...
void print_query_time(std::ofstream& out) {
	LocalDateTime now;
	out << "\nQuery " << DateTimeFormatter::format(now, "%d.%m.%y %H:%M:%S") << "\n";
}
...
json print_doctors() {
	...
	if (resp["status"] == "ok") {
		if (file.open(QIODevice::Append)) {
			QTextStream stream(&file);
			stream << QString::fromUtf8("Доктора: ") << "\n";
...
json print_cards() {
	...
	if (resp["status"] == "ok") {
		if (file.open(QIODevice::Append)) {
			QTextStream stream(&file);
			stream << QString::fromUtf8("Пациенты: ") << "\n";
...
std::string print_schedules() {
	...
	if (resp["status"] == "ok") {
		...
		QTextStream stream(&file);
		stream << QString::fromUtf8("Формат расписания: день недели, начало приема, конец приема, длительность приема одного пациента\n");
		for (auto& x : schedule) {
...
json print_talons() {
	...
	if (resp["status"] == "ok") {
		std::string row;
		logging << QString::fromUtf8("Формат: id, начало приема, конец приема, дата, id доктора\n").toLocal8Bit().toStdString();
